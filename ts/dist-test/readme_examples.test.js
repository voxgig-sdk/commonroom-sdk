...
function isRunnable(code) {
    return (/new\s+CommonroomSDK\b/.test(code) ||
        /\bCommonroomSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+CommonroomSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/CommonroomSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
