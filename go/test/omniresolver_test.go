...
import (
	...
	omni "github.com/voxgig-sdk/commonroom-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CommonroomError); is {
				out := map[string]any{
					"name":    "CommonroomError",
...
func omniCtx(arg any, client *sdk.CommonroomSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CommonroomSDK); is {
			client = live
