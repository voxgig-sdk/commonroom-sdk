...
func TestMemberDirect(t *testing.T) {
	t.Run("direct-list-member", func(t *testing.T) {
		...
		if setup.live {
			for _, _liveKey := range []string{"community01"} {
	...
	t.Run("direct-load-member", func(t *testing.T) {
		...
		if setup.live {
			...
			listParams["community_id"] = setup.idmap["community01"]
...
func memberDirectSetup(mockres any) *memberDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"COMMONROOM_TEST_MEMBER_ENTID": map[string]any{},
	...
	if live {
		...
		client := sdk.NewCommonroomSDK(mergedOpts)

		idmap := map[string]any{}
		if entidRaw, ok := env["COMMONROOM_TEST_MEMBER_ENTID"]; ok {
