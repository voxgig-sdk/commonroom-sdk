...
func TestMemberEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set COMMONROOM_TEST_MEMBER_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func memberBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"member01", "member02", "member03", "community01"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("COMMONROOM_TEST_MEMBER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["COMMONROOM_TEST_MEMBER_ENTID"])
	...
	if env["COMMONROOM_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewCommonroomSDK(core.ToMapAny(mergedOpts))
	}
