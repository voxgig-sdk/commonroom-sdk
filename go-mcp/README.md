...
[MCP](https://modelcontextprotocol.io) server exposing the Commonroom SDK as
two agent tools — `commonroom_list` and `commonroom_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/commonroom-mcp)
make build

# 2. Provide credentials via the environment
export COMMONROOM_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user commonroom \
  -- /absolute/path/to/commonroom-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./commonroom-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// commonroom_list: first page of records
{ "entity": "member" }
{ "entity": "member", "query": { } }

// commonroom_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/commonroom-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user commonroom \
     -- "$PWD"/dist/*/commonroom-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `commonroom_list` and `commonroom_load` tools now appear
   in new sessions. Ask the agent to *"list member using commonroom"*
   and it calls `commonroom_list` with `{"entity":"member"}`.
...
```sh
export COMMONROOM_APIKEY=sk_live_xxx            # API key
export COMMONROOM_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `commonroom_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `commonroom_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `COMMONROOM_APIKEY` | API key sent with every request. |
| `COMMONROOM_BASE` | Optional override of the API base URL. |
...
```sh
./commonroom-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"commonroom_load","arguments":{"entity":"member","query":{"id":1}}}}'
```
