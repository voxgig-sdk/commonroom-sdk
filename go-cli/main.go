// Command Commonroom-cli is a CLI + REPL for the Commonroom SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      COMMONROOM_APIKEY (api key), COMMONROOM_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: COMMONROOM_APIKEY carries the API key and
	// COMMONROOM_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("COMMONROOM_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("COMMONROOM_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCommonroomSDK(opts)
