...
boru-driven command-line client **and** interactive REPL for the Commonroom
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/commonroom-cli)
make build
...
./commonroom-cli list member
./commonroom-cli load 1 member            # {id:1} shorthand
./commonroom-cli load '{id:1}' member       # explicit match map

# 5. Override the API base URL for a single call
COMMONROOM_BASE=https://api.example.com ./commonroom-cli list member

# 6. No arguments -> interactive REPL
./commonroom-cli
commonroom> list member
commonroom> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/commonroom-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/commonroom-cli list member
   ```
...
```sh
./commonroom-cli load 1 member          # scalar shorthand for {id:1}
./commonroom-cli load '{id:1}' member     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `commonroom>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/commonroom-cli`. |
...
(`./commonroom-cli <expr>`) and interactively (the REPL), and expressions compose the
