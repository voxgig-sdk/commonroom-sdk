...
class CommonroomSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = CommonroomHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = CommonroomHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!CommonroomFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, CommonroomFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return CommonroomUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = CommonroomHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = CommonroomHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = CommonroomHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new CommonroomError($op . "_allow",
                "CommonroomSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = CommonroomHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new CommonroomError("graphql_error",
                "CommonroomSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
