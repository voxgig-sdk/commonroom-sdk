...
class FtCtrl extends CommonroomControl
{
...
class FtClient
{
    ...
    private CommonroomUtility $utility;

    public function __construct(CommonroomUtility $utility, array $options)
    {
    ...
    public function get_utility(): CommonroomUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CommonroomContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new CommonroomUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (CommonroomContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CommonroomConfig::make_feature($name);
    ...
    private static function buildUrl(CommonroomSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CommonroomError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CommonroomResponse($fetched);
        }
    ...
    private function fail(CommonroomContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CommonroomContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
