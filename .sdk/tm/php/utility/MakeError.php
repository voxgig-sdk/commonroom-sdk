...
class CommonroomMakeError
{
    public static function call(?CommonroomContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new CommonroomContext([], null);
        }
        $op = $ctx->op ?? new CommonroomOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new CommonroomResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof CommonroomError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new CommonroomError('', $msg, $ctx);
        ...
        if ($err instanceof CommonroomError) {
